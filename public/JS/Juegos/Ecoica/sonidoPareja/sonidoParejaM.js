document.addEventListener('DOMContentLoaded', function() {
    const SEGUNDOS = 50;
    const palabras = ["Campana", "Tambor", "Rana", "Zapato", "Perro", "Pato", "Humano", "Leon", "Acordeon"];
    let pares = [];
    let score = 0;
    let seleccionados = [];
    let timer = null;
                ...
                if (seleccionados.length < 2 && !btn.classList.contains('found') && !btn.classList.contains('selected')) {
                    ...
                    if (seleccionados.length === 2) {
                        ...
                        if (seleccionados[0].palabra === seleccionados[1].palabra) {
                            ...
                            document.getElementById('feedback').textContent = "No coinciden.";
                            setTimeout(() => {
                                seleccionados.forEach(sel => {
                                    sel.btn.classList.remove('selected');
                                });
    ...
    function showIntro() {
        ...
        govMsg.innerHTML = `Más parejas.<br>Tienes <strong>${SEGUNDOS} segundos</strong>.`;
    ...
    function showGameOver(win) {
        ...
        if (win) {
            ...
            govTitle.textContent = "¡Reto Superado!";
            govMsg.innerHTML = "Tu memoria auditiva es muy buena.";
            actionBtn.textContent = "Jugar de nuevo";
        } else {
            govBubble.className = "speech-bubble lose-theme";
            govEyebrow.textContent = "TIEMPO AGOTADO";
            govTitle.textContent = "Casi lo tienes";
            govMsg.innerHTML = "Intenta memorizar la ubicación de los sonidos.";

// DIFICULTAD: DIFÍCIL - palabras cortas, enemigos chicos
const diccionario = [
...
class Enemy {
    constructor(x, y, radius, color, velocity, text, fontSize = 16) {
...
function spawnEnemy() {
    enemyInterval = setInterval(() => {
        ...
        const radius = parabraRandom.length * 3;
        ...
        const velocity = {x: Math.cos(angle) * 1.1, y: Math.sin(angle) * 1.1};
        enemies.push(new Enemy(x, y, radius, color, velocity, parabraRandom, 14));
    }, 800);// aqui es el tiempo de aparicion de los enemigos
}

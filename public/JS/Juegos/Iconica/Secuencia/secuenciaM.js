...
function pressReady() { faseMemorizar.style.display = 'none';faseRecordar.style.display = 'block'; }
...
function pressSlot(indiceSlot) {
    ...
    for (let i = 0; i < imagenesPiscina.length; i++) {
        if (imagenesPiscina[i] == idImagen) {
            document.getElementById("pool_" + i).style.visibility = 'visible';break;
        }
    }
}
...
if (restartBtn) restartBtn.addEventListener('click', function(){
    score=0; vidas=3; nivelActual=1; modalGameover.style.display='none';
    faseMemorizar.style.display='flex'; faseRecordar.style.display='none';initRonda();
});
nivelActual=1;initRonda();
